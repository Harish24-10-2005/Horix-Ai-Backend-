...
def build_user_context(credentials: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    """
    try:
        user = _show_current_user(credentials)
        if not user:
        ...
        if context["team_ids"]:
            service_ids = services.fetch_service_ids(team_ids=context["team_ids"],credentials = credentials)
            context["service_ids"] = [
...
def show_user(*, user_id: str,credentials: Optional[Dict[str, Any]] = None,) -> Dict[str, Any]:
...
def _show_current_user(credentials: Optional[Dict[str, Any]] = None,) -> Dict[str, Any]:
